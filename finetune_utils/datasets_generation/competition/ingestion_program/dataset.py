...
try:
    import src.datasets_creation.competition.ingestion_program.dataset_utils as dataset_utils

    from src.datasets_creation.competition.ingestion_program.data_pb2 import DataSpecification
    from src.datasets_creation.competition.ingestion_program.data_pb2 import MatrixSpec
except ImportError:
    ...
    """
        ...
        for i in range(self.metadata_.get_bundle_size()):
            if self.metadata_.is_sparse(i):
                sequence_features[self._feature_key(i, "sparse_col_index")] = tf.io.VarLenFeature(
                    tf.int64
                )
                sequence_features[self._feature_key(i, "sparse_row_index")] = tf.io.VarLenFeature(
                    tf.int64
                )
                sequence_features[self._feature_key(i, "sparse_channel_index")] = tf.io.VarLenFeature(
                    tf.int64
                )
                sequence_features[self._feature_key(i,
                                                    "sparse_value")] = tf.io.VarLenFeature(tf.float32)
            elif self.metadata_.is_compressed(i):
                sequence_features[self._feature_key(i, "compressed")] = tf.io.VarLenFeature(tf.string)
            else:
                sequence_features[self._feature_key(i, "dense_input")] = tf.io.FixedLenSequenceFeature(

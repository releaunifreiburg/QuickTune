...
class AFTMetaDataset:
    ...
    def aggregate_curves(self):
        ...
        for name in self.curves_names:
            with open(f"{self.path}/agg_curves/{self.set}/{name}.json", 'w') as outfile:
                json.dump(aggregated_curves[name], outfile)

        with open(f"{self.path}/agg_curves/{self.set}/args.json", 'w') as outfile:
                json.dump(aggregated_args, outfile)

    def load_curves(self, curve_names):
        self.curves = {}
        for name in curve_names:
            with open(f"{self.path}/agg_curves/{self.set}/{name}.json", 'r') as stream:
    ...
    def load_metafeatures(self):
        ...
        #read json file
        with open(f"{self.path}/agg_curves/dataset-meta-features/{superset}/meta-features.json", 'r') as stream:
    ...
    def load_args(self, preprocess=False, file_name="unnormalized_args_table"):
        ...
        if not os.path.exists(f"{self.path}/agg_curves/{self.set}/{file_name}.csv"):
            with open(f"{self.path}/agg_curves/{self.set}/args.json", 'r') as stream:
            ...
            self.args_df.to_csv(f"{self.path}/agg_curves/{self.set}/{file_name}.csv")
        else:
            self.args_df = pd.read_csv(f"{self.path}/agg_curves/{self.set}/{file_name}.csv", index_col=0)
...
if __name__ == "__main__":
    ...
    target_model = None
    action_on_model = None
    loader = AFTMetaDataset(aggregate_data=aggregate_data,
                            set=set,

...
class CostPredictorTrainer:
    ...
    def get_batch_for_finetuning(self):
        ...
        curves = torch.FloatTensor(curves).to(self.device)/1000
    ...
    def get_batch(self, mode = 'train', dataset = None):
        ...
        for _ in range(self.batch_size):
            ...
            temp_curves = []
            curve_eval = self.metadataset.get_curve(hp_index=hp_index, budget=budget, curve_name="eval_time")
            curve_train = self.metadataset.get_curve(hp_index=hp_index, budget=budget, curve_name="train_time")
            curve = [curve_eval[i] + curve_train[i] for i in range(len(curve_eval))]
            budget = len(curve)
            target = curve[-1]
            curve = curve[:-1]
        ...
        for i in range(self.train_iter):
            ...
            if i % self.test_freq == 0:
                ...
                if val_error < min_eval_error:
                    ...
                    self.model.save_checkpoint()
                    print("Saving checkpoint with error: ", min_eval_error)
